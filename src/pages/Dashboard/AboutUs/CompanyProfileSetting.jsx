...
} from "../../../Redux/companyProfile/companyProfileApi";
...
const CompanyProfileSetting = () => {
  ...
  const [updateCompanyProfile, { isLoading: updateLoading }] =
    useUpdateCompanyProfileMutation();
  const [createCompanyProfile, { isLoading: createLoading }] =
    useCreateCompanyProfileMutation();
  ...
  const updateAboutUsHandler = async (e) => {
    ...
    if (id) {
      try {
        const res = await updateCompanyProfile({ id, formData }).unwrap();
        if (res?.success) {
          setImage([]);
      ...
      try {
        const res = await createCompanyProfile(formData).unwrap();
        if (res?.success) {
          setImage([]);
          Swal.fire("", "Profile add success", "success");
        }
      } catch (error) {
        Swal.fire("", error?.data?.error, "error");
      }
  ...
  return (
    <section className="bg-base-100 rounded shadow">
      <div className="p-4 border-b">
        <h3 className="font-medium text-neutral">Company Profile</h3>
      </div>
