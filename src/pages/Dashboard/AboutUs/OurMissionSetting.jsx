...
import { useCreateOurMissionMutation, useGetOurMissionsQuery, useUpdateOurMissionMutation } from "../../../Redux/ourMission/ourMissionApi";

const OurMissionSetting = () => {
  const editor = useRef(null);

  const { data, isLoading } = useGetOurMissionsQuery();
  const [updateOurMission, { isLoading: updateLoading }] =
  ...
  const updateAboutUsHandler = async (e) => {
    ...
    if (id) {
      try {
        const res = await updateOurMission({ id, formData }).unwrap();
        if (res?.success) {
          setImage([]);
      ...
      try {
        ...
        if (res?.success) {
          setImage([]);
          Swal.fire("", "Mission add success", "success");
        }
      } catch (error) {
        Swal.fire("", error?.data?.error, "error");
      }
  ...
  return (
    <section className="bg-base-100 rounded shadow">
      <div className="p-4 border-b">
        <h3 className="font-medium text-neutral">Our Mission</h3>
      </div>
...
export default OurMissionSetting;
