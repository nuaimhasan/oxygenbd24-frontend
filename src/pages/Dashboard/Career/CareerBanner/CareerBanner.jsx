...
} from "../../../../Redux/careerBanners/careerBannerApi";
...
export default function CareerBanner() {
  ...
  const [updateCareerBanner, { isLoading: updateLoading }] =
    useUpdateCareerBannerMutation();
  const [createCareerBanner, { isLoading: createLoading }] =
    useCreateCareerBannerMutation();
  ...
  const updateAboutUsHandler = async (e) => {
    ...
    if (id) {
      try {
        const res = await updateCareerBanner({ id, formData }).unwrap();
        if (res?.success) {
          setImage([]);
      ...
      try {
        const res = await createCareerBanner(formData).unwrap();
        if (res?.success) {
          setImage([]);
